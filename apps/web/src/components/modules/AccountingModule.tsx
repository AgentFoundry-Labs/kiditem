...
export default function AccountingModule() {
  ...
  const invoiceColumns: Column<typeof taxInvoices[0]>[] = [
    ...
    },
    {
      key: 'company',
      header: '거래처',
      render: (item) => <span className="text-gray-700">{item.company}</span>,
    },
    {
      key: 'amount',
      header: '금액',
      align: 'right',
      render: (item) => <span className="text-gray-700 font-medium">{formatCurrency(item.amount)}</span>,
    },
    {
      key: 'status',
      header: '상태',
    ...
    },
    {
      ...
      align: 'center',
      render: (item) => (
        <span className={`text-[10px] ${item.method === '자동' ? 'text-blue-600' : 'text-gray-500'}`}>
          {item.method}
        </span>
      ),
    },
      ...
      <div className="glass-card p-5">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-gray-900">자금일보</h3>
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-green-50 border border-emerald-500/20 text-green-600 text-xs hover:bg-emerald-500/20 transition-colors"
          >
        ...
        <div className="grid grid-cols-4 gap-3 mb-4">
          <MetricCard label="전일잔액" value={formatCurrency(fundDaily.openingBalance)} color="text-gray-700" />
          <MetricCard label="입금합계" value={formatCurrency(totalDeposit)} color="text-green-600" icon={<ArrowDownRight className="w-4 h-4 text-green-600" />} />
          ...
          <MetricCard label="당일잔액" value={formatCurrency(closingBalance)} color="text-blue-600" />
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="rounded-xl bg-gray-50 border border-gray-200 p-3">
            <p className="text-[10px] text-green-600 font-semibold mb-2 uppercase tracking-wider">입금 내역</p>
            <div className="space-y-1.5">
              {fundDaily.deposits.map((d, i) => (
                <div key={i} className="flex items-center justify-between py-1.5 px-2 rounded-lg hover:bg-gray-50">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[11px] text-green-600 font-medium">+{formatCurrency(d.amount)}</span>
      ...
      <div className="glass-card p-5">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-gray-900">입금 매칭 현황</h3>
          </div>
          <div className="flex items-center gap-2">
          ...
          {paymentMatches.map((pm) => (
            ...
            >
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs text-gray-700 font-medium">{pm.company}</span>
                {pm.isMatched ? (
