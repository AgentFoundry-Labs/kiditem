  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-3">
        <MetricCard label="기간 총 매출" value={formatCurrency(totalRevenue)} color="text-green-600" trend={{ value: 12, label: '전기비' }} />
        <MetricCard label="총 주문수" value="487건" color="text-blue-600" trend={{ value: 8, label: '전기비' }} />
          ...
          <div className="space-y-1.5 mt-2">
            {platformPieData.map((p) => (
              <div key={p.name} className="flex items-center justify-between">
                ...
                <span className="text-[10px] text-gray-500">{Math.round(p.value / totalRevenue * 100)}%</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Top Products */}
      <div className="glass-card p-5">
        <h3 className="text-sm font-semibold text-gray-900 mb-4">판매 TOP 10</h3>
        <div className="space-y-2">
          {topProducts.map((p) => (
            <div key={p.rank} className="flex items-center gap-4 py-2 px-3 rounded-lg hover:bg-gray-50 transition-colors">
              ...
              <span className="text-xs text-gray-700 font-medium w-24 text-right">{formatCurrency(p.revenue)}</span>
              <span className={`text-[10px] w-12 text-right ${p.growth > 0 ? 'text-green-600' : p.growth < 0 ? 'text-red-400' : 'text-gray-500'}`}>
                {p.growth > 0 ? '+' : ''}{p.growth}%
              </span>
            </div>
          ))}
      ...
      <div className="glass-card p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-semibold text-gray-900">월말보고서 아카이브</h3>
        </div>
        <div className="space-y-2">
          {monthlyReports.map((r) => (
            <div key={r.month} className="flex items-center justify-between p-3 rounded-lg bg-gray-50 border border-gray-200">
              <div className="flex items-center gap-3">
                <FileText className="w-4 h-4 text-violet-400" />
                <div>
                  <p className="text-xs text-gray-700 font-medium">{r.month} 월말보고서</p>
                  <p className="text-[10px] text-gray-600">생성일: {r.generatedAt} | {r.pages}페이지</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <StatusBadge variant="success">완료</StatusBadge>
                <button className="p-1.5 rounded-lg hover:bg-gray-50 text-gray-500 transition-colors">
