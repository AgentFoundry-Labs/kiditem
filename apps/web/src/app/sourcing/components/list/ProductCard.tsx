  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-slate-900/80 p-3 pt-6 z-0 flex justify-between items-center text-white">
      ...
      <div className="p-4 bg-slate-50">
        ...
        <h3
          className="text-sm font-bold text-slate-800 mb-3 line-clamp-1"
          title={product.name}
        >
          {product.name}
        </h3>
        ...
        <div className="flex justify-between items-center mb-1">
          <span className="text-xs text-slate-500">원가 (CNY)</span>
        ...
        <div className="flex gap-2">
          <button
            onClick={() => onNavigate(product.id)}
            className="flex-1 flex items-center justify-center gap-1.5 py-2 bg-slate-100 text-slate-600 rounded-lg text-xs font-bold hover:bg-slate-200 transition-colors"
          >
            <Eye size={12} /> 상세
          </button>
          <button
