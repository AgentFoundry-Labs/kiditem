...
`app/(media-ai)/_shared/hooks/useThumbnailGenerations.ts:14-18`:
```typescript
...
`useSelectCandidate()` (app/(media-ai)/_shared/hooks/useThumbnailGenerations.ts:29-42):
...
- `app/(media-ai)/_shared/hooks/useThumbnailGenerations` — media-ai 그룹 공유 hook (thumbnail-editor 와 함께 사용)
...
| Polling cadence 변경 | `app/(media-ai)/_shared/hooks/useThumbnailGenerations.ts:14-18` (interval 함수) |
