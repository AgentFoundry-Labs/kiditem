...
페이지 헤더의 `ProductSelector` 로 productId 를 URL `?productId=...` 동기화. 각 슬롯은 `SlotCard` → `ImageSourceDrawer` 를 통해 상품 콘텐츠 이미지 자산, 업로드, 이전 생성 이미지, 다른 상품 이미지를 선택한다. 자산 이미지는 role 필터로 표시하고 클릭 시 slot setter 로 반영한다.
