...
export async function collectTeachervilleXlsxFromExtension(): Promise<{
  ...
  if (!res?.success || !res.xlsxBase64) {
    throw new Error(res?.error ?? '티쳐몰 주문 수집에 실패했습니다.');
  }
...
export async function convertTeachervilleToSellpiaFile(
  ...
  const outName =
    fileNameFromContentDisposition(response.headers.get('Content-Disposition')) ?? '티쳐몰_셀피아변환.xls';
...
async function readPreviewRows(blob: Blob): Promise<string[][]> {
  ...
  return rows.slice(0, 24).map((row) => row.slice(0, 36).map((cell) => String(cell ?? '')));
}
