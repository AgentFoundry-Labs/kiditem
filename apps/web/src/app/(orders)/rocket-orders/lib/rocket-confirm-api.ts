...
export async function listRocketPosFromExtension(
  ...
  const res = await sendToExtension<{ success?: boolean; pos?: RocketPoSummary[]; error?: string }>(
    extensionId,
    { action: 'listRocketPos', from, to, status: statusCode, runId: globalThis.crypto.randomUUID() },
    70000,
  );
...
/** 재고 매칭 방식: barcode(정확) · name(이름) · name-fuzzy(유사, 확인필요) · null(미매칭). */
export type RocketMatchKind = 'barcode' | 'name' | 'name-fuzzy';
