...
describe("projectAgentRunGraph", () => {
  ...
  it("uses capability domains for requestless dot-form tool lanes", () => {
    ...
    expect(
      result.lanes
        .find((lane) => lane.id === "sourcing")
        ?.nodes.map((node) => node.id),
    ).toContain("tool:tool-score-dot-1");
    expect(
      result.lanes
        .find((lane) => lane.id === "channel_registration")
        ?.nodes.map((node) => node.id),
    ).toEqual(["tool:tool-channel-dot-1"]);
  });
  ...
  it("returns the public graph, lane, and node fields expected by canvas consumers", () => {
    ...
    const listingTask = getExecutionCanvasNode(
      result,
      "task:request-listing-1",
    );
  ...
  it("normalizes backend statuses into the small canvas status vocabulary", () => {
    ...
    expect(toExecutionCanvasStatus("claimed")).toBe("running");
    ...
    expect(toExecutionCanvasStatus("waiting_approval")).toBe(
      "waiting_approval",
    );
    expect(toExecutionCanvasStatus("requires_approval")).toBe(
      "waiting_approval",
    );
  ...
  it("finds a node by id for right-panel detail rendering", () => {
    ...
    expect(
      getExecutionCanvasNode(result, "artifact:artifact-listing-1"),
    ).toMatchObject({
      kind: "artifact",
      label: "쿠팡 등록 준비 패키지",
      status: "succeeded",
