...
interface ReturnTransfer {
  ...
  notes: string | null;
  createdAt: string;
  inventorySku: { sellpiaProductCode: string; name: string; optionName: string | null };
}

const EMPTY_FORM = { inventorySkuId: '', quantity: 1, condition: 'good', notes: '' };
    ...
    <section className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--text-secondary)]">회수·검수 사실을 물리 InventorySku에 기록합니다. Sellpia 반영 전까지 현재고는 바뀌지 않습니다.</p>
        </div>
        <button type="button" onClick={() => setShowForm(true)} className="inline-flex items-center gap-2 rounded-lg bg-[var(--primary)] px-4 py-2 text-sm font-medium text-white"><Plus className="h-4 w-4" aria-hidden="true" /> 반품 기록 추가</button>
      </div>
      {isLoading ? <p className="py-10 text-center text-[var(--text-secondary)]">불러오는 중...</p> : (
        <div className="overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--surface)]"><div className="overflow-x-auto"><table className="w-full min-w-[820px]"><thead><tr><th>R/T 번호</th><th>Sellpia SKU</th><th className="text-right">수량</th><th>상태</th><th>품질</th><th>기록 시각</th></tr></thead><tbody>{transfers.length ? transfers.map((transfer) => <tr key={transfer.id}><td className="font-mono text-xs">{transfer.rtNumber}</td><td><p className="font-medium">{transfer.inventorySku.name}</p><p className="font-mono text-xs text-[var(--text-secondary)]">{transfer.inventorySku.sellpiaProductCode} · {transfer.inventorySku.optionName ?? '옵션 없음'}</p></td><td className="text-right">{formatNumber(transfer.quantity)}개</td><td>{transfer.status}</td><td>{transfer.condition}</td><td className="text-sm text-[var(--text-secondary)]">{formatDateTime(transfer.createdAt)}</td></tr>) : <tr><td colSpan={6} className="py-12 text-center text-[var(--text-secondary)]">반품 기록이 없습니다.</td></tr>}</tbody></table></div></div>
      )}
      {showForm ? (
        <div className="modal-overlay" onClick={() => setShowForm(false)}><div className="modal-content max-w-lg" onClick={(event) => event.stopPropagation()}>
          ...
          <div className="space-y-4">
            <InventorySkuPicker value={form.inventorySkuId} onChange={(inventorySkuId) => setForm((current) => ({ ...current, inventorySkuId }))} label="Sellpia 반품 상품" />
