...
/**
 * detail-template generation + collected-products detail — KidsPlayful 생성 이력 카드 리스트.
 *
...
} from '@/app/(product-pipeline)/product-pipeline/_shared/components/detail-page/DownloadOptionsModal';
import {
...
import {
  HISTORY_PREVIEW_MODAL_STYLE,
  HISTORY_PREVIEW_VIEWPORT_WIDTH,
  buildHistoryPreviewHtml,
} from '../lib/history-preview-html';
...
function FullscreenViewer({ entry, onClose }: FullscreenViewerProps) {
  ...
  } = useKidsPlayfulOne(entry.id);
  const previewEntry = latestEntry ?? entry;
  const renderEntry = latestEntry ?? null;
  ...
  const html = useMemo((): string | null => {
    ...
    if (!renderEntry || !canRenderGeneratedResult(renderEntry)) return null;
    if (isBoldVertical) {
      const adapted = adaptBoldVerticalToDetailPageData(
        renderEntry.result as unknown as BoldVerticalGeneration,
        renderEntry.imageUrls,
        renderEntry.processedImages,
        API_BASE,
      );
      return buildBoldVerticalHtml(adapted, templateCss);
    }
    return buildKidsPlayfulHtml(rowToRendererData(renderEntry), templateCss);
  }, [editedHtml, editedHtmlLoaded, isBoldVertical, renderEntry, templateCss]);
  ...
  const syncPreviewContentHeight = useCallback(() => {
    try {
      ...
      // Scripted srcDoc previews run in an opaque sandboxed origin.
      // The preview bridge posts authoritative metrics instead.
    }
  }, []);
      ...
      >
        ...
        <div data-history-preview-scroll className="flex-1 overflow-y-auto overflow-x-hidden bg-white">
          {isPreviewLoading ? (
            ...
            <div className="flex min-h-full w-full justify-start">
              <iframe
                ref={iframeRef}
