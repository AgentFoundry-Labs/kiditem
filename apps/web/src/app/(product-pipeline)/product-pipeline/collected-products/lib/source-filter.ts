export const SOURCING_SOURCE_FILTERS = [
  ...
  { key: 'manual-registration', label: '상품 등록', platform: 'KIDITEM_PRODUCT_REGISTRATION' },
] as const;
...
export function emptyStateCopyForSourceFilter(filter: SourcingSourceFilter): {
  ...
  if (filter === 'manual-registration') {
    return {
      title: '상품 등록 후보가 없습니다.',
      description: '상품 등록에서 이미지와 정보를 입력하면 후보 카드가 여기에 만들어집니다.',
    };
  }
  if (filter === '1688') {
    return {
  ...
  return {
    title: '수집된 상품이 없습니다.',
    description: '1688 수집 또는 상품 등록으로 첫 후보를 등록해 보세요.',
  };
}
