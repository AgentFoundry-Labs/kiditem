# web/thumbnail-ai — Smart Polling + Batch + Optimistic UI

**6 탭 dashboard + dynamic refetchInterval + AbortController 배치 cancel**. 무거운 분석 jobs 를 실시간으로 추적.
...
│                        # AnalysisKpiCards, GradeDonutChart 등
├── hooks/               # 도메인 hooks (useThumbnailAnalysis/Tracking/BatchAnalysis/Actions). 공유 generations hook 은 ../_shared/hooks/useThumbnailGenerations.ts
└── lib/                 # coupang-wing.ts, resolve-url.ts
```
...
`useSelectCandidate()` (`app/(product-pipeline)/product-pipeline/_shared/hooks/useThumbnailGenerations.ts`):
...
page.tsx:74-115 — `batchAbortRef` + `batchCancelRef` ref 로 진행 중
배치를 추적한다. 사용자 cancel 은 AbortController, 서버 cancel, UI 즉시
반영을 함께 수행한다.
...
- Grade 색상은 `../_shared/lib/thumbnail-grade.ts` 의 `gradeBg` map 경유
...
- `app/(product-pipeline)/product-pipeline/_shared/hooks/useThumbnailGenerations` — product-pipeline 공유 hook (thumbnail-generation 과 함께 사용)
...
| Polling cadence 변경 | `app/(product-pipeline)/product-pipeline/_shared/hooks/useThumbnailGenerations.ts` (interval 함수) |
| Grade 색 매핑 변경 | `../_shared/lib/thumbnail-grade.ts` + 모든 컴포넌트 (GradeDonutChart, 공유 ScoreBreakdown 등) |
