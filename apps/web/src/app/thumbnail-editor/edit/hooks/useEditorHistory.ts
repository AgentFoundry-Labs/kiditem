...
export function useEditorHistory({
  ...
  const historyCandidates = useMemo<HistoryCandidate[]>(() => {
    ...
    if (productId) {
      const productGens = allGenerations
        .filter((g) => g.productId === productId)
        .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
      for (const gen of productGens) {
    ...
    return list;
  }, [allGenerations, productId, result, mode, generationId]);

  const recommendedCandidateUrl = useMemo(() => {
    if (!productId) return null;
    const scored = allGenerations.filter(
      (g) => g.productId === productId && typeof g.score === 'number' && g.score > 0,
    );
