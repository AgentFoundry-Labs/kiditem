...
import { needsThumbnailFix } from '@/app/thumbnails/lib/thumbnail-classification';
...
export function NeedsFixSection() {
  ...
  const gotoEdit = (r: ThumbnailAnalysisResult, selection: FeatureSelection) => {
    const params = new URLSearchParams({ productId: r.productId });
    if (r.imageUrl) params.set('imageUrl', r.imageUrl);
    params.set('mode', selection.mode);
    if (selection.mode === 'edit') params.set('editCase', selection.editCase);
    if (selection.mode === 'creative') {
      if (selection.scene) params.set('scene', selection.scene);
      if (selection.customPrompt) params.set('customPrompt', '1');
    }
