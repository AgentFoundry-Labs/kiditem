...
describe('<RecipeSuggestionDialog>', () => {
  it('loads a read-only proposal and never applies a recipe mutation', async () => {
    vi.mocked(apiClient.getParsed).mockResolvedValue({
      ...
      status: 'high_confidence_name', automationDecision: 'auto_apply', recommendedQuantity: 2,
      reason: '고신뢰 상품명 일치', existingComponents: [],
      proposals: [{ sellpiaInventorySkuId: '55555555-5555-4555-8555-555555555555', code: 'SP-1', name: '본품', optionName: null, currentStock: 4, evidence: [{ kind: 'fuzzy_name', channelValue: '본 품 2개', normalizedValue: '본품', score: 0.91 }], requiresQuantityConfirmation: false, recommendedQuantity: 2 }],
    });
    ...
    expect(screen.getByText('고신뢰 상품명 확정')).toBeInTheDocument();
    expect(screen.getByText('추천 수량 2')).toBeInTheDocument();
    expect(screen.getByText('fuzzy_name: 본 품 2개 · 신뢰도 91%')).toBeInTheDocument();
