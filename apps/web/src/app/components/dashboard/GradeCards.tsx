...
export default function GradeCards({ gradeCount, warnings, totalProducts }: GradeCardsProps) {
  return (
    <>
        ...
        {(['A', 'B', 'C'] as const).map(g => {
          ...
          return (
            <Link key={g} href={config.href} className={cn('bg-white rounded-xl border border-gray-200 border-l-4 p-4 hover:shadow-md transition-all', config.left)}>
              ...
              <div className="text-xs text-gray-400 mt-1">{pct}% of {totalProducts}</div>
            </Link>
          );
        })}
      </div>
      ...
      {/* Warning Cards */}
      <div className="grid grid-cols-3 gap-3">
        <Link href="/products?grade=C" className="bg-white rounded-xl border border-gray-200 border-l-4 border-l-red-500 p-4 hover:shadow-md transition-all">
        ...
        <Link href="/products?grade=C" className="bg-white rounded-xl border border-gray-200 border-l-4 border-l-orange-500 p-4 hover:shadow-md transition-all">
