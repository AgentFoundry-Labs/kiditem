    ...
    return (
      <div className="card p-6">
        <div className="text-center py-4">
          <p className="text-sm text-slate-400">평가 데이터 없음</p>
        </div>
      </div>
    );
  }
    ...
    <div className="card p-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-base font-semibold text-slate-900">상품 진단 현황</h3>
        </div>
        {healthSummary.lastEvaluatedAt && (
          <span className="text-xs text-slate-400">{timeAgo(healthSummary.lastEvaluatedAt)} 평가</span>
        )}
      </div>
      <div className="flex items-center gap-6 mb-4">
        <div className="flex items-center gap-2">
          <span className="w-2.5 h-2.5 rounded-full bg-green-500" />
          <span className="text-sm text-slate-700">정상 <strong className="text-green-700">{healthSummary.healthy}</strong>개</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-2.5 h-2.5 rounded-full bg-amber-500" />
          <span className="text-sm text-slate-700">주의 <strong className="text-amber-700">{healthSummary.warning}</strong>개</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-2.5 h-2.5 rounded-full bg-red-500" />
          <span className="text-sm text-slate-700">위험 <strong className="text-red-700">{healthSummary.critical}</strong>개</span>
        </div>
      </div>
      ...
      {healthSummary.topCritical && healthSummary.topCritical.length > 0 && (
        <div className="border-t border-slate-100 pt-3">
          <p className="text-xs font-medium text-slate-400 mb-2">위험 상품 TOP {healthSummary.topCritical.length}</p>
            ...
            {healthSummary.topCritical.slice(0, 5).map((item) => (
               <div key={item.id} className="flex items-center justify-between">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  {item.abcGrade && (
                    <span className="text-xs text-slate-400">{item.abcGrade}등급</span>
                  )}
                </div>
