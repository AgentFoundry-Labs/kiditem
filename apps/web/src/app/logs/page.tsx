...
export default function LogsPage() {
  ...
  const columns: Column<ExecutionLog>[] = [
    ...
    },
    {
      key: 'workflowName',
      header: '워크플로우',
      render: (item) => <span className="text-gray-700 font-medium">{item.workflowName}</span>,
    },
    {
      key: 'message',
      header: '메시지',
      render: (item) => <span className="text-gray-500 truncate max-w-[300px] block">{item.message}</span>,
    },
    {
      key: 'statusBadge',
      header: '상태',
  ...
  return (
    <div className="space-y-6 max-w-[1200px]">
      ...
      <div className="grid grid-cols-4 gap-3">
        <MetricCard label="오늘 총 실행" value={formatNumber(stats.todayExecutions)} color="text-blue-600" icon={<BarChart3 className="w-4 h-4" />} />
        <MetricCard label="성공" value={successCount} color="text-green-600" icon={<CheckCircle className="w-4 h-4" />} />
