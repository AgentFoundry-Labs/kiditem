  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-[11px] uppercase tracking-[0.18em] text-slate-400">AGENT</p>
          <h2 className="text-xl font-semibold text-slate-900 mt-1">{agent.name}</h2>
          <p className="text-sm text-slate-500 mt-1">월간 UTC 예산</p>
        </div>
        <div className={cn(
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="border border-slate-200 rounded-lg p-4">
          <p className="text-[11px] uppercase tracking-[0.18em] text-slate-400">Observed</p>
          <p className="text-xl font-semibold text-slate-900 mt-2 tabular-nums">{formatCost(observedCents)}</p>
          <p className="text-xs text-slate-400 mt-1">
            {budgetCents > 0 ? `${progressPct.toFixed(1)}% 사용` : '한도 없음'}
          </p>
        </div>
        <div className="border border-slate-200 rounded-lg p-4">
          <p className="text-[11px] uppercase tracking-[0.18em] text-slate-400">Budget (tokens)</p>
      ...
      {/* Token stats */}
      {runtimeState && (
        <div className="border border-slate-200 rounded-lg p-4 grid grid-cols-3 gap-4">
          <div>
            <span className="text-[11px] uppercase tracking-wide text-slate-400 block mb-1">입력 토큰</span>
            <span className="text-base font-semibold tabular-nums text-slate-900">{formatTokens(runtimeState.totalInputTokens)}</span>
          </div>
          <div>
            <span className="text-[11px] uppercase tracking-wide text-slate-400 block mb-1">출력 토큰</span>
            <span className="text-base font-semibold tabular-nums text-slate-900">{formatTokens(runtimeState.totalOutputTokens)}</span>
          </div>
          <div>
      ...
      <div className="border border-slate-200 rounded-lg p-4 space-y-3">
        <label className="text-[11px] uppercase tracking-[0.18em] text-slate-400 block">
          월간 토큰 예산 설정
        </label>
        <div className="flex gap-2">
          <input
            type="number"
            className="flex-1 px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-slate-400 focus:border-slate-400 transition-colors"
          ...
          />
          <button
            className="flex items-center gap-1.5 px-4 py-2 text-sm bg-slate-900 text-white rounded-lg hover:bg-slate-700 disabled:opacity-50 transition-colors whitespace-nowrap"
