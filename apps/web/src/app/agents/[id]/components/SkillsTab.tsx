    ...
    <div className="space-y-4 max-w-2xl">
      ...
      {skills.length === 0 ? (
        <div className="border border-slate-200 rounded-lg p-8 text-center">
          ...
          <p className="text-xs text-slate-400 mt-1">에이전트 설정에서 스킬을 추가할 수 있습니다.</p>
        </div>
      ) : (
        <div className="border border-slate-200 rounded-lg overflow-hidden">
          {skills.map((skill, index) => (
            ...
            >
              <div className="w-3.5 h-3.5 rounded border-2 border-slate-300 bg-white flex items-center justify-center shrink-0">
                <div className="w-1.5 h-1.5 rounded-sm bg-blue-500" />
              </div>
              <div className="flex-1 min-w-0">
                <span className="text-sm font-medium text-slate-900 font-mono">{skill}</span>
      ...
      <div className="border border-slate-100 rounded-lg p-4 bg-slate-50">
        <p className="text-xs text-slate-500">
          <span className="font-medium text-slate-700">스킬 관리:</span> 스킬은{' '}
          <code className="font-mono text-slate-700 bg-white px-1 rounded border border-slate-200">agent-config/skills/</code>{' '}
          디렉토리에서 심링크로 주입됩니다.
        </p>
      </div>
    </div>
