          ...
          <tbody>
            ...
            {filtered.map((p) => (
              <tr key={p.id} className={p.adRateOverLimit ? "bg-red-50/50" : ""}>
                <td><span className={cn('px-2 py-0.5 rounded text-xs font-bold', getGradeColor(p.grade))}>{p.grade}</span></td>
                ...
                <td className={cn('text-right font-semibold', ROAS_STATUS_COLOR[p.roasStatus])}>{p.roas}%</td>
                <td className="text-right">{p.ctr}%</td>
                <td className="text-right">{p.convRate}%</td>
                <td className={cn('text-right font-semibold', p.adRateOverLimit ? 'text-red-600' : 'text-slate-600')}>{formatPercent(p.adRate)}</td>
                <td className={cn('text-right', p.profitRate < 0 ? 'text-red-600 font-bold' : p.profitRate <= 3 ? 'text-orange-500' : 'text-green-600')}>{formatPercent(p.profitRate)}</td>
                <td>
                  {p.adRateOverLimit && <span className="px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-800">점검필요</span>}
                  {p.roasStatus === 'poor' && !p.adRateOverLimit && <span className="px-2 py-0.5 rounded text-xs font-medium bg-orange-100 text-orange-800">효율낮음</span>}
                  {!p.adRateOverLimit && (p.roasStatus === 'excellent' || p.roasStatus === 'good') && <span className="px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-800">정상</span>}
                </td>
              </tr>
            ))}
          </tbody>
