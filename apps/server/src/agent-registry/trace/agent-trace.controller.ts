 ...
 * AgentTrace 관측 엔드포인트.
 *
 * - ADR-0006: `@CurrentOrganization()` 로만 organizationId 주입. 쿼리/바디 organizationId 금지.
 ...
 *       UUID 로 파싱 시도해 500.
 *   v2: `agent-tasks` — organizations/agent-tasks.controller.ts 가 동일 prefix 선점.
...
export class AgentTraceController {
  ...
  list(
    ...
    return this.svc.listTasks(organizationId, q);
  }
  ...
  trace(
    ...
    return this.svc.getTrace(params.id, organizationId, q);
  }
}
