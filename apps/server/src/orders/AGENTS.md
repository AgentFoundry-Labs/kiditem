...
단일 리소스 GET / PATCH / DELETE 는 `findUnique({ id })` 절대 금지. 항상 `findFirst({ where: { id, companyId } })` 사용 (`apps/server/AGENTS.md` 멀티테넌트 격리 규칙). `OrderLineItem` / `OrderReturnLineItem` 의 `companyId` denormalize 도 같은 목적 — line 레벨 접근에서도 companyId 검증 가능.
