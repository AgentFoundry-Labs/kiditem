...
- `Order` — `@@unique([organizationId, platform, externalOrderId])` (Coupang 의 경우 `externalOrderId = shipmentBoxId`, `externalNumber = orderId`)
- `OrderLineItem` — `@@unique([orderId, externalLineId])` (Coupang 의 경우 `externalLineId` 는 Coupang `vendorItemId` 값이 들어가며, 캐노니컬 매핑은 `ChannelListingOption.externalOptionId` 경유)
...
하나의 `OrdersModule` 이 5 controller (orders/returns/cs/reviews/return-transfers) 등록. Cross-service 의존 없음. `return-transfers` 는 Wave H1 fold 로 합류했지만 여전히 stock 미변경 record-only 동작이며, stock movement invariant 는 `inventory/AGENTS.md` 의 record-only 규칙 + integration spec `#8` 가 lock-in.
...
### 3. IDOR 방어 — organizationId 조합 필수

단일 리소스 GET / PATCH / DELETE 는 `findUnique({ id })` 절대 금지. 항상 `findFirst({ where: { id, organizationId } })` 사용 (`apps/server/AGENTS.md` 멀티테넌트 격리 규칙). `OrderLineItem` / `OrderReturnLineItem` 의 `organizationId` denormalize 도 같은 목적 — line 레벨 접근에서도 organizationId 검증 가능.

### 4. CS DTO — `listingId` vs `productId` backward compat alias (Plan B2c.orders T2)

3-layer product schema 로 Product 계층이 drop 되면서 `CSRecord.listingId` (nullable) 로 재배선. 프런트 일괄 rewire 이전 legacy 호출자 호환을 위해 `CreateCsBodyDto` 는 두 필드를 모두 받는다.
...
- **유지 정책**: `productId` 는 compatibility alias 로만 유지한다. 새 호출자/프론트 코드는 `listingId` 를 보내야 하며, alias 제거는 호출자 grep + scoped plan 으로 안전하게 검증될 때만 진행한다. 그 전까지 `@deprecated` JSDoc 유지 + 로깅 금지 (정상 alias 경로).

확인 포인트: DTO 에 `productId` 필드 추가/변경 시 service resolver 의 우선순위 가드도 같이 점검. `productId` 는 legacy alias 이고 새 코드에서는 쓰지 말 것.
...
- ❌ 신규 channel-specific 테이블 (`NaverOrder` 등) 추가. `platform` 값 추가로 해결.
- ❌ `findUnique({ id })` 로 리소스 GET — IDOR. 항상 `findFirst({ id, organizationId })`.
