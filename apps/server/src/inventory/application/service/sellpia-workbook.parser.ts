...
export function parseSellpiaWorkbook(buffer: Buffer): ParsedSellpiaWorkbook {
  ...
  try {
    // ⭐Sellpia 재고 엑셀은 구형 BIFF .xls + EUC-KR(CP949) 인코딩이라 codepage 949 로 디코딩해야 한다.
    // 안 주면 헤더가 모지바케(예: '상품코드'→'»óÇ°ÄÚµå')가 되어 record['상품코드'] 매칭이 전부 실패 → 정규화 안 됨.
    // (.xlsx 업로드 시엔 codepage 무시되어 무해.)
    workbook = XLSX.read(buffer, { type: 'buffer', codepage: 949 });
  } catch {
