...
describe('Inventory architecture contract', () => {
  it('uses InventorySku identity for record-only fulfillment models', () => {
    ...
    for (const modelName of ['StockTransfer', 'PickingItem', 'ReturnTransfer']) {
      ...
      expect(block, `${modelName} must not retain ProductOption identity`).not.toMatch(/\boptionId\b/);
      expect(block, `${modelName} must not retain ProductOption relation`).not.toContain('ProductOption');
    }
  });
