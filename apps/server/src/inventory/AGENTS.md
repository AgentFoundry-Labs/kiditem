...
backend architecture contract in `apps/server/AGENTS.md`: **Domain-first
modular architecture with Application orchestration and selective Hexagonal
Ports**. PR #146 finalised the port/adapter naming used here.
...
│       ├── inventory.service.ts              (single writer invariant)
...
`BundleStockService.recomputeForComponent` 를 호출한다 (single-writer
