...
describe('ThumbnailEditorAiService reference prompt parity', () => {
  ...
  it('validates generated bytes and stores the detected format', async () => {
    const { service, generatedImageValidator, storage } = makeService();

    const candidates = await service.generateCreative(
      [makeInput()],
      ORGANIZATION_ID,
      { model: 'gemini-image-direct' },
    );
