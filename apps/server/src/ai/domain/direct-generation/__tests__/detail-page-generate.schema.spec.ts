...
describe('DetailPageGenerateDirectOutputSchema', () => {
  ...
  it('accepts a bold-vertical output with the minimum required fields', () => {
    const parsed = DetailPageGenerateDirectOutputSchema.safeParse({
...
describe('DetailPageGenerateDirectInputSchema', () => {
  it('fills in default heroImageMode when missing', () => {
    const parsed = DetailPageGenerateDirectInputSchema.parse({
  ...
  it('rejects unknown templateId', () => {
    const parsed = DetailPageGenerateDirectInputSchema.safeParse({
      templateId: 'unknown',
      raw: { rawTitle: 'product' },
    });
    expect(parsed.success).toBe(false);
