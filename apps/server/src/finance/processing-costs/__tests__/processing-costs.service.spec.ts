 ...
 *
 * Coverage:
 *   findAll  — organizationId in WHERE; status filter composes
 *   create   — master FK pre-check is organization-scoped; organizationId + masterId in
 ...
 *   monthly  — organizationId in WHERE
 *
...
describe('ProcessingCostsService', () => {
  ...
  describe('findAll', () => {
    it('always scopes by organizationId with master include', async () => {
    ...
    it('composes status filter without dropping organizationId', async () => {
      ...
      expect(prisma.processingCost.findMany).toHaveBeenCalledWith({
        where: { organizationId: COMPANY_A, status: 'pending' },
        include: { master: true },
        orderBy: { createdAt: 'desc' },
      });
    });
  ...
  describe('create', () => {
    it('writes organizationId from argument and computes totalCost', async () => {
    ...
    it('cross-organization denial: rejects when masterId is not owned by caller organization', async () => {
  ...
  describe('monthly', () => {
    it('aggregates organization-scoped rows only', async () => {
      prisma.processingCost.findMany.mockResolvedValue([
