...
describe('SalesAnalysisService.getAnalysis (PG integration)', () => {
  ...
  it('groups orders by channel (coupang + naver)', async () => {
    const coup = await setupChannelFixture(TEST_COMPANY_ID, 'coupang', 'GROUP-COUP');
    const naver = await setupChannelFixture(TEST_COMPANY_ID, 'naver', 'GROUP-NAVER');
    await seedOrderWithLineItems(prisma, {
      companyId: TEST_COMPANY_ID, externalOrderId: 'GROUP-1', orderedAt: '2026-04-10T00:00:00Z',
      lineItems: [{ quantity: 1, totalPrice: 10000, optionId: coup.optionId, listingOptionId: coup.listingOptionId }],
    });
    await seedOrderWithLineItems(prisma, {
      companyId: TEST_COMPANY_ID, externalOrderId: 'GROUP-2', orderedAt: '2026-04-15T00:00:00Z',
  ...
  it('IDOR double-blind — TEST + OTHER companies returns each own data', async () => {
    const tcoup = await setupChannelFixture(TEST_COMPANY_ID, 'coupang', 'IDOR-T-COUP');
    const ocoup = await setupChannelFixture(OTHER_COMPANY_ID, 'coupang', 'IDOR-O-COUP');
    await seedOrderWithLineItems(prisma, {
      companyId: TEST_COMPANY_ID, externalOrderId: 'IDOR-T1', orderedAt: '2026-04-10T00:00:00Z',
      lineItems: [{ quantity: 1, totalPrice: 10000, optionId: tcoup.optionId, listingOptionId: tcoup.listingOptionId }],
    });
    await seedOrderWithLineItems(prisma, {
      companyId: OTHER_COMPANY_ID, externalOrderId: 'IDOR-O1', orderedAt: '2026-04-10T00:00:00Z',
  ...
  it('ADR-0017 returnRate — past-period order excluded from current returnRate', async () => {
    const coup = await setupChannelFixture(TEST_COMPANY_ID, 'coupang', 'ADR-0017');
  ...
  it('orphanReturnCount — orderId NULL returns go to totals.orphanReturnCount', async () => {
    const coup = await setupChannelFixture(TEST_COMPANY_ID, 'coupang', 'ORPHAN');
    await seedOrderWithLineItems(prisma, {
      companyId: TEST_COMPANY_ID, externalOrderId: 'ORPHAN-1', orderedAt: '2026-04-10T00:00:00Z',
      lineItems: [{ quantity: 1, totalPrice: 10000, optionId: coup.optionId, listingOptionId: coup.listingOptionId }],
    });
  ...
  it('empty-channel ad — spend on channel with 0 orders is dropped', async () => {
    const coup = await setupChannelFixture(TEST_COMPANY_ID, 'coupang', 'EMPTY-C');
    const naver = await setupChannelFixture(TEST_COMPANY_ID, 'naver', 'EMPTY-N');
    // coupang has orders; naver only has ad spend (no orders)
    await seedOrderWithLineItems(prisma, {
      companyId: TEST_COMPANY_ID, externalOrderId: 'EMPTY-1', orderedAt: '2026-04-10T00:00:00Z',
      lineItems: [{ quantity: 1, totalPrice: 10000, optionId: coup.optionId, listingOptionId: coup.listingOptionId }],
    });
