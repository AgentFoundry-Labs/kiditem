...
describe('AdBenchmark flow (PG integration)', () => {
  ...
  async function seedBenchmarkConfig(organizationId: string) {
    ...
    await prisma.systemSetting.createMany({
      data: [
        { organizationId, key: 'ads.benchmark.roas', value: { avg: 350, good: 500, excellent: 700, poor: 200 } },
        { organizationId, key: 'ads.benchmark.ctr', value: { avg: 0.3, good: 0.5, excellent: 1.0, poor: 0.15 } },
        { organizationId, key: 'ads.benchmark.cvr', value: { avg: 8, good: 12, excellent: 15, poor: 5 } },
      ],
      skipDuplicates: true,
    });
  }
  ...
  beforeEach(async () => {
    ...
    await seedBenchmarkConfig(TEST_ORGANIZATION_ID);
  });

  describe('getDiagnosis — ownMetrics + industryAverage shape', () => {
    it('#1 Ad 없음 → ownMetrics 모두 0/null + listings 빈 배열', async () => {
      const result = await service.getDiagnosis(TEST_ORGANIZATION_ID);
  ...
  describe('cross-tenant scope', () => {
    it('#11 OTHER_ORGANIZATION_ID 의 Ad 는 TEST_ORGANIZATION_ID 결과에 포함되지 않음', async () => {
