...
describe('AdExecutionService tenant-scoped runtime writes', () => {
  ...
  it('lease scopes task leasing through the owning action company and worker update through companyId', async () => {
    ...
    expect(tx.executionTask.updateMany).toHaveBeenCalledWith({
      where: { id: 'task-1', status: 'queued', action: { companyId: COMPANY_ID } },
  ...
  it('report scopes task and action updates to the request company', async () => {
    const { prisma, tx } = makePrisma();
    const service = new AdExecutionService(prisma as never);
    ...
    expect(tx.executionTask.updateMany).toHaveBeenCalledWith({
      where: { id: 'task-1', action: { companyId: COMPANY_ID } },
      data: expect.objectContaining({
        status: 'done',
        errorMessage: null,
      }),
    ...
    expect(tx.adAction.updateMany).toHaveBeenCalledWith({
      where: { id: 'action-1', companyId: COMPANY_ID },
      data: expect.objectContaining({
        executeStatus: 'done',
        errorMessage: null,
      }),
