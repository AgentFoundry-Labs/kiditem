...
root. The owner-domain shape follows
...
domain-first modular architecture with selective hexagonal ports, applied
where it removes coupling rather than as a global dogma.
...
- Raw SQL paths (dashboard only): `${companyId}::uuid` per ADR-0018 with a
