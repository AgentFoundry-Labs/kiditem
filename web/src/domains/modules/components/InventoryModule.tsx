  ...
  const columns: Column<typeof mockInventory[0]>[] = [
    {
      key: 'sku',
      header: 'SKU',
      render: (item) => <span className="font-mono text-[11px] text-blue-400">{item.sku}</span>,
    },
    {
      key: 'productName',
      header: '상품명',
      render: (item) => <span className="text-gray-300">{item.productName}</span>,
    },
    {
      key: 'selpiaStock',
      header: '셀피아',
      align: 'center',
      render: (item) => (
        <span className={item.selpiaStock === 0 ? 'text-red-400 font-bold' : item.selpiaStock < 10 ? 'text-amber-400' : 'text-gray-300'}>
          {item.selpiaStock}
        </span>
      ),
    },
    {
      ...
      align: 'center',
      render: (item) => (
        <span className={item.isMismatch ? 'text-amber-400' : 'text-gray-300'}>
          {item.symphonyStock}
        </span>
      ),
    },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-card p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-sm font-semibold text-white">재고 동기화</h3>
      ...
      <div className="grid grid-cols-4 gap-3">
        <MetricCard label="전체 SKU" value={totalItems} icon={<Package className="w-4 h-4" />} color="text-white" />
        ...
        <MetricCard label="불일치" value={mismatches} color={mismatches > 0 ? 'text-red-400' : 'text-emerald-400'} subValue="셀피아 ↔ 심포니" />
      </div>
            ...
            {mockInventory.filter((i) => i.isMismatch).slice(0, 6).map((item) => (
              <div key={item.id} className="flex items-center justify-between p-2 rounded-lg bg-amber-500/5 border border-amber-500/10">
                <div className="min-w-0">
                  <p className="text-[11px] text-gray-300 truncate">{item.productName}</p>
