...
export default function CSModule() {
  ...
  const columns: Column<typeof csInquiries[0]>[] = [
    ...
    },
    {
      key: 'content',
      header: '내용',
      render: (item) => <span className="text-gray-300 truncate max-w-[200px] block">{item.content}</span>,
    },
    {
      key: 'status',
      header: '상태',
    ...
    },
    {
      ...
      render: (item) => (
        <div className="flex items-center gap-1 justify-center">
          ...
          <span className={cn('text-[11px]', item.handler === 'AI' ? 'text-blue-400' : 'text-gray-500')}>
            {item.handler}
          </span>
        </div>
      ),
    },
    {
      ...
      render: (item) =>
        item.aiConfidence ? (
          <span className={cn('text-[11px] font-medium', item.aiConfidence >= 90 ? 'text-emerald-400' : 'text-amber-400')}>
            {item.aiConfidence}%
          </span>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-3">
        ...
        <MetricCard label="AI 자동답변" value={autoReplied} subValue={`${Math.round(autoReplied / csInquiries.length * 100)}% 자동처리율`} color="text-blue-400" icon={<Bot className="w-4 h-4" />} />
        <MetricCard label="평균 응답시간" value={`${avgResponseTime}분`} subValue="목표: 30분 이내" color="text-emerald-400" icon={<MessageSquare className="w-4 h-4" />} />
      ...
      <div className="glass-card p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-sm font-semibold text-white">AI 자동답변 미리보기</h3>
        </div>
          ...
          {csInquiries.filter((c) => c.status === 'auto_replied').slice(0, 3).map((inquiry) => (
            <div key={inquiry.id} className="p-3 rounded-lg bg-blue-500/5 border border-blue-500/10">
              ...
              <div className="flex gap-3">
                <div className="flex-1 p-2 rounded bg-black/20 border border-[#1e2028]">
                  <p className="text-[10px] text-gray-600 mb-1">고객 문의</p>
                  <p className="text-xs text-gray-400">{inquiry.content}</p>
                </div>
                <div className="flex-1 p-2 rounded bg-blue-500/5 border border-blue-500/10">
                  <p className="text-[10px] text-blue-400 mb-1">AI 답변</p>
      ...
      <div className="glass-card p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-sm font-semibold text-white">품절 안내 발송 현황</h3>
        </div>
        <div className="grid grid-cols-2 gap-3">
          {outOfStockNotices.map((notice, i) => (
            <div key={i} className="flex items-center justify-between p-3 rounded-lg bg-red-500/5 border border-red-500/10">
              <div>
                <p className="text-xs text-gray-300 font-medium">{notice.product}</p>
                <p className="text-[10px] text-gray-600">{notice.platform} | {notice.date}</p>
              </div>
              <div className="flex items-center gap-3">
                <div className="text-right">
                  <p className="text-xs text-emerald-400">{notice.sent}건 발송</p>
                  {notice.pending > 0 && (
