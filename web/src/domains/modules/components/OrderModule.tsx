  ...
  const columns: Column<typeof mockOrders[0]>[] = [
    {
      ...
      header: '주문번호',
      render: (item) => (
        <span className="text-blue-400 font-mono text-[11px]">{item.orderNumber}</span>
      ),
    },
    {
      key: 'platform',
    ...
    },
    {
      ...
      header: '상품명',
      render: (item) => (
        <span className="text-gray-300 truncate max-w-[180px] block">{item.productName}</span>
      ),
    },
    {
      key: 'quantity',
    ...
    },
    {
      ...
      align: 'right',
      render: (item) => (
        <span className="text-gray-300 font-medium">{formatCurrency(item.amount)}</span>
      ),
    },
    {
      key: 'status',
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-card p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h3 className="text-sm font-semibold text-white">주문수집 제어</h3>
            {lastCollected && (
          ...
          <div className="flex items-center gap-2">
            ...
            <button className="flex items-center gap-2 px-4 py-2 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-medium hover:bg-emerald-500/20 transition-colors">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs text-blue-400">셀피아 + 사방넷 주문수집 중...</span>
                  <span className="text-[10px] text-gray-600">6/8 채널</span>
                </div>
                <div className="h-1 bg-[#1a1d26] rounded-full overflow-hidden">
      ...
      <div className="glass-card p-5">
        <h3 className="text-sm font-semibold text-white mb-4">플랫폼별 현황</h3>
        <div className="grid grid-cols-4 gap-3">
          {platformStats.map((p) => (
            <div key={p.name} className="flex items-center gap-3 p-3 rounded-lg bg-black/20 border border-[#1e2028]">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ backgroundColor: `${p.color}15` }}>
                <div className="w-3 h-3 rounded-full" style={{ backgroundColor: p.color }} />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium text-gray-300">{p.name}</p>
      ...
      <DataTable
        ...
        actions={
          <div className="flex items-center gap-2">
            <select className="px-3 py-1.5 bg-[#0d0e13] border border-[#1e2028] rounded-lg text-xs text-gray-400 focus:outline-none">
