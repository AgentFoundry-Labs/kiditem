# Plan B2b — Advertising Service Listing Migration (Design Spec v2)

> **Status**: Draft v2 — 3-reviewer adversarial review 반영 (critic PASS-WITH-CHANGES / code-reviewer APPROVE-WITH-CHANGES / architect NEEDS-REVISION minor)
> **Session**: 2026-04-18
...
1. **Scope 확장** — `ad-execution.service.ts`, `ad-config.service.ts`, `ad-collect.service.ts` 추가 (전부 `getDefaultCompanyId()` 보유). `ad-sync.getExtensionStatus` (line 52) 의 `prisma.product.count` 도 rewrite 포함.
2. **AdAction 규칙 정확화** — v1 의 listing-level 5 규칙 설명 폐기. **실제 snapshot-level 5 규칙 port** (stock=0 예산컷 / keyword pause / keyword bid 하향 / 캠페인 예산 확대 / 캠페인 예산 축소) + 현행 하드코딩 thresholds 명시 (5000 KRW / roas 100/200/480 / bid×0.85 / budget×1.2 or ×0.5).
3. **필드명 수정** — `ChannelListingOption.externalId` → **`ChannelListingOption.vendorItemId`** (section 4.1).
4. **companyId param 위치** — Plan B2a 관례 따라 **last parameter** 로 통일.
5. **select 최적화** — `masterProduct: { select: { id, code, name } }` — rawData/processedData/draftContent Json 과다 로드 방지.
6. **Rule 5 (재고 0) 처리** — `Ad.optionId`/`AdSnapshot.optionId` null 이면 Rule 1 (기존 넘버링) skip. Stock 은 option 레벨 concept.
7. **DTO 데코레이터 강화** — `@IsUUID()` for listingId, `@IsUUID() @IsOptional()` for optionId.
8. **handleActionCommand companyId 전파** — 7 sub-calls (`generate`/`approve`/`reject`/`markRunning`/`markDone`/`markFailed`/`resetFailed`) 전부 명시.
9. **registerCampaign error contract** — `listingId` 가 companyId 에 속하지 않으면 `NotFoundException`.
10. **ad-config 저장소 정정** — `BusinessRule` 아니라 **`SystemSetting`** 사용.
11. **`AD_ACTION_TARGET_TYPES` const** — `services/types.ts` 에 `['campaign', 'keyword'] as const` 선언 (Plan B2c 의 execution worker 가 dispatch 기준으로 사용).
12. **ad-strategy 분할 정량 트리거** — post-port LOC > 1400 시 Plan B2b.refactor 강제.
13. **`resolvePricing` / `resolveInventory` import 제거** — `ad-strategy.service.ts:8` 의 import 삭제. 파일 자체는 유지 (B2c 에서 전체 삭제).
14. **RegisterCampaignDto 배열 shape 이전** — `products: [{productId, productName}]` → `listings: [{listingId, label?}]`. Frontend 도 이 이름에 맞춰야 함 (Plan D).
15. **ExecutionTask 커버리지 gap 명시** — ad-execution 변경 없음 → AdAction → ExecutionTask 파이프라인 integration test 는 B2b 에서 커버 안 함.
16. **`ad-flow.spec.ts` 대체 확인** — 삭제되는 cross-module mock 테스트의 agent-registry 쪽 `onResultReady` 검증은 기존 `apps/server/src/agent-registry/domains/ad-strategy/__tests__/ad-strategy.service.spec.ts` 가 커버 (확인 완료).
17. **Rollback 정책** — Plan D 가 지연돼도 shared 스키마 rewrite 는 merge. `apps/web` 은 그때까지 build 실패 상태 (CI 는 server + shared 만 검증, apps/web build 는 Plan D 까지 skip).
18. **Frontend TS error 규모 (섹션 3.3)** — apps/web 는 `productId` 필드 의존 8+ 곳에서 타입 에러. B2b 에서 count 안 하지만 Plan D 에서 한 번에 대응.
...
| Path | 작업 |
|---|---|
| `apps/server/src/advertising/controllers/advertising.controller.ts` | **수정** — 각 handler 에 `@CurrentCompany() companyId: string` 주입 (14 handlers 전부 포함 — `getConfig` / `updateConfig` / `getHub` / `changeTier` / `getCampaigns` / `getTrends` / `registerCampaign` / `getRules` / `getWeeklyPlan` / `getAiPlan` / `getRecommendations` / `getExposureAnalysis` / `getBenchmark` / `startCollection` / `getCollectStatus` / `extensionSync` / `extensionStatus` / `getScrapeTargets` / `handleScrapeTarget` / `deleteScrapeTarget` / `getActions` / `handleActionCommand` / `executionLease` / `executionHeartbeat` / `executionReport` / `findAll`). `handleActionCommand` 는 7 sub-calls (`generate`/`approve`/`reject`/`markRunning`/`markDone`/`markFailed`/`resetFailed`) 전부에 companyId 전달 필수 — IDOR 방지. `findAll(query as any)` 캐스트 제거 (신 `ListAdsQueryDto` 로 타입 맞춤). 라우트 구조 불변 |
...
| `apps/server/src/advertising/services/ad-strategy.service.ts` | **재작성** (1139 LOC — 이번 plan 최대 surface). `getRules` / `getWeeklyPlan` / `getAiEnhancedPlan` / `getRecommendations` / `getExposureAnalysis` / `registerCampaign` 전부 listing-primary. 모든 internal `calc*` (calcActions, calcAdIssues, calcTierAnalysis, calcTop20, calcSnapshotKeyMetrics, calcBudgetAllocation) listing 단위. 5 score 계산 메서드 (sales/review/ad/fulfillment/info) 시그니처 유지. `determineTopIssue` 는 listing-level input 으로 동작. `Alert.productId` 제거 (Alert schema 는 targetType/targetId 이미 있음 — `targetType:'listing'` + `targetId:listingId`). **`resolvePricing` / `resolveInventory` import 제거** (line 8 의 `from '../../common/master-product-resolver'` 삭제). `common/master-product-resolver.ts` 파일은 유지 (Plan B2c 에서 전체 삭제). `registerCampaign` 은 `listingId` in `listings[]` 각각에 대해 `channelListing.findFirst({id, companyId, isDeleted:false})` 선행 검증 → 없으면 `NotFoundException`. **서비스 분할은 이번 plan 범위 밖** — 1 파일 1139 LOC 유지. **정량 트리거**: post-port LOC > 1400 이면 Plan B2b.refactor 강제 (플랜 작성 시 측정) |
...
| `apps/server/src/advertising/services/ad-sync.service.ts` | **재작성** (707 LOC). `buildProductMap` → `buildListingMap(companyId)`: `{ externalIdMap: Map<string, {listingId}>, vendorItemMap: Map<string, {listingId, optionId}> }` 2 lookup 테이블. `matchProductIdFromRow` → `matchListingFromRow(row, map)`: 우선순위 (1) `vendorItemId` 히트 → `ChannelListingOption.vendorItemId` 매칭 → `{listingId, optionId}` (2) `externalId` 히트 → `ChannelListing.externalId` (+ `platform='coupang'` + `isDeleted=false`) 매칭 → `{listingId, optionId: null}` (3) 매칭 실패 → AdSnapshot 은 저장 (`rawJson` + `listingId=null`), Ad/TrafficStats 는 생성 안 함. `handleAdCampaign` / `handleRawScrape` / `handleTraffic` / `handleCoupangAdsDaily` 모두 신 매칭 사용. **`getExtensionStatus`** (line 49-97, 별도 메서드) 내부 `prisma.product.count` (line 52) → `prisma.channelListing.count({ where: { companyId, isDeleted: false } })` 로 전환. buildListingMap 은 request 당 1회 (per-sync lifetime) — 10k listings 기준 ~8MB in-memory, 수용 가능 |
| `apps/server/src/advertising/services/ad-action.service.ts` | **재작성** (502 LOC). 기존 snapshot-level 5 규칙 **port** (redesign 아님) + `snapshot.productId` → `snapshot.listingId`, `snapshot.product.inventory.currentStock` → option/listing stock resolution 경로만 교체. 규칙 목록 (섹션 4.5 에 상세): Rule 1 (stock=0 캠페인 예산컷), Rule 2 (keyword pause: 전환0 AND spend≥5000 OR roas∈(0,100)), Rule 3 (keyword bid 하향: roas∈[100,200)), Rule 4 (캠페인 예산 확대: grade=A AND roas≥480), Rule 5 (캠페인 예산 축소: grade=C OR roas<100, dailyBudget>3000). **현행 하드코딩 thresholds 모두 보존** (5000 KRW / roas 100/200/480 / bid×0.85 / budget×1.2 / budget×0.5 / min 3000). `AdAction.listingId` (nullable) + `targetType ∈ {'campaign', 'keyword'}` (listing 은 targetType 값 아님). `include: { listing: { include: { masterProduct: { select: { id, code, name } } } }, snapshot: true }` 패턴. `getActions` / `approveActions` / `rejectActions` / `markRunning` / `markDone` / `markFailed` / `resetFailed` IDOR fix (`findFirst({id, companyId})`). Rule 1 재고 판정 시 `snapshot.optionId` null 이면 skip (listing-level snapshot 은 재고 판정 불가 — 옵션 레벨 concept) |
| `apps/server/src/advertising/services/ad-execution.service.ts` | **ADR-0006 compliance only** (0 listing errors). `getDefaultCompanyId()` 제거 (line 18). `lease/heartbeat/report` 3 메서드에 `companyId` 말미 파라미터 추가. ExecutionWorker/Task schema 무관 — lookup/스냅샷 기록만 |
| `apps/server/src/advertising/services/ad-config.service.ts` | **ADR-0006 compliance only** (0 listing errors). `getDefaultCompanyId()` 제거 (line 45). `getConfig(companyId)` / `updateConfig(companyId, key, value)` 시그니처로 통일. 저장소: `prisma.systemSetting` (NOT BusinessRule) — 기존 그대로 |
| `apps/server/src/advertising/services/ad-collect.service.ts` | **ADR-0006 compliance only** (0 listing errors). `getDefaultCompanyId()` 제거 (line 11). `startCollection(period, companyId)` / `getStatus(companyId)` 시그니처 |
| `apps/server/src/advertising/services/types.ts` | **수정**. `GradeBudgetAllocation` 및 기타 service-internal type 을 listingId 기반으로 재정의. `productId` 필드 제거. **신규 export**: `export const AD_ACTION_TARGET_TYPES = ['campaign', 'keyword'] as const; export type AdActionTargetType = typeof AD_ACTION_TARGET_TYPES[number];` — AdAction 생성 시 이 union 사용 (Plan B2c 의 execution worker dispatch 기준) |
| `apps/server/src/advertising/dto/ad-action.dto.ts` | **수정**. `productId` 필드 제거 (쿼리 필터). `listingId?: string` (filter) + `optionId?: string` (filter) 추가. `@IsUUID() @IsOptional()` 데코레이터 필수 (class-validator 가 non-UUID 거부). Command DTO 는 기존 유지 |
| `apps/server/src/advertising/dto/register-campaign.dto.ts` | **재작성**. 기존 `RegisterCampaignProductDto { productId: string; productName: string }[]` 배열 shape 폐기 → `RegisterCampaignListingDto { @IsUUID() listingId: string; @IsString() @IsOptional() label?: string }[]` 배열. 상위 `listings: RegisterCampaignListingDto[]` (기존 `products` 필드명도 **rename**). `campaignName` / `dailyBudget` / `adGroupName` / `grade` / `operationMode` 등 나머지 필드 유지. 프론트엔드 `useRegisterCampaign` 은 Plan D 에서 신 shape 로 이관 |
...
- dashboard/: ~13 (B3 대기)
- finance/: 잔여 (B3 대기)
- **server 잔존**: ~15 errors, 전부 dashboard/finance (Plan B3 scope). `dev:server` 부팅 **이번 plan 에서 목표 아님** (B2c/B3 완료 후).

**packages/shared**: 0 errors (신 Zod schemas + z.infer types 빌드 PASS).

**apps/web**: **일시 깨짐 허용**. 신 `ads.ts` 가 배포되면 ad-ops 페이지의 8+ 곳에서 `productId` 참조가 타입 에러:
- `apps/web/src/app/ad-ops/hooks/useAdOpsData.ts:68` (`products: {productId, productName}[]`)
- `apps/web/src/app/ad-ops/components/StrategyContent.tsx` (grade-level recommendations)
- `apps/web/src/app/ad-ops/components/ProductDrilldown.tsx` (product-level drill)
- `apps/web/src/app/ad-ops/components/RegisterCampaignModal.tsx` (register payload)
- `apps/web/src/app/ad-ops/components/ExposureAnalysis.tsx` (urgent actions)
- 외 KpiDashboard / CampaignContent / StatusContent 등
Plan D 에서 일괄 재배선. `apps/web` 의 `npm run build` 는 Plan D 완료까지 CI gate 에서 제외 (또는 advisory 로 표시).
...
`buildListingMap(companyId)` 한 번 빌드 후 sync 전체 수명 동안 재사용:

```ts
// buildListingMap(companyId) 반환:
//   {
//     vendorItemMap: Map<vendorItemId, { listingId, optionId }>
//     externalIdMap: Map<externalId, { listingId }>
//   }
//
// vendorItemMap 은 ChannelListingOption 전수 조회 (isDeleted:false) → vendorItemId 키.
// externalIdMap 은 ChannelListing 전수 조회 (isDeleted:false, platform='coupang') → externalId 키.

// matchListingFromRow(row, map):
// Priority 1: vendorItemId (가장 정확)
//   - vendorItemMap.get(row.vendor_item_id) → { listingId, optionId }
// Priority 2: externalId (쿠팡 등록상품ID)
//   - externalIdMap.get(row.external_id) → { listingId, optionId: null }
// Priority 3: 매칭 실패
//   - AdSnapshot 은 저장 (rawJson + listingId=null + optionId=null) — 디버깅/재매칭 자료
//   - Ad 는 생성 안 함 (Ad.listingId 필수)
//   - TrafficStats 는 생성 안 함 (TrafficStats.listingId 필수)
```

**주의**: Prisma 필드명 `channelListingOption.vendorItemId` (NOT `externalId` — v1 오기). `ChannelListing.externalId` (listing-level 필드) 와 `ChannelListingOption.vendorItemId` (option-level 필드) 는 **서로 다른 컬럼**.
...
- ad-campaigns / ad-strategy 는 `ad.groupBy({ by: ['listingId'], _sum: {spend, revenue, impressions, clicks, conversions} })` 후 **select 최적화 필수**:
  ```ts
  channelListing.findMany({
    ...
    select: {
      ...
      // NOT: rawData, processedData, draftContent Json fields (OOM 위험)
    },
  })
  ```
  N+1 회피 — listingIds 전체를 `in` 절로 한 번에 조회.
...
### 4.5 AdAction 5 Rules (port, not redesign)

기존 `ad-action.service.ts:362-461` 의 snapshot-level 5 규칙을 **그대로 port** 한다. productId → listingId 외의 로직 변경 없음. 하드코딩 thresholds 모두 보존 (A/B 테스트 없이 바꾸면 광고 실행 결과에 직접 영향).

`AdAction.listingId` (nullable, SetNull) + `AdAction.targetType ∈ {'campaign', 'keyword'}`:

| Rule | 조건 | actionType | targetType | priority |
|---|---|---|---|---|
| **1. 재고 0 예산컷** | `snapshot.pageType='campaign' AND stock===0 AND dailyBudget>0` (option 매칭 실패 시 stock 판정 불가 → skip) | `change_daily_budget` | `campaign` | `urgent` (proposedValue=3000) |
| **2. 키워드 pause** | `snapshot.pageType='keyword' AND !isPaused(status) AND ((conversions===0 AND spend>=5000) OR (roas∈(0,100)))` | `pause_keyword` | `keyword` | grade=A: `high` / else `urgent` |
| **3. 키워드 bid 하향** | `snapshot.pageType='keyword' AND currentBid>0 AND roas∈[100,200) AND nextBid<currentBid` where `nextBid=roundBid(currentBid*0.85)` | `change_bid` | `keyword` | profitRate<0: `high` / else `medium` |
| **4. 캠페인 예산 확대** | `snapshot.pageType='campaign' AND dailyBudget>0 AND grade='A' AND roas>=480 AND nextBudget>dailyBudget` where `nextBudget=roundBudget(dailyBudget*1.2)` | `change_daily_budget` | `campaign` | `high` |
| **5. 캠페인 예산 축소** | `snapshot.pageType='campaign' AND dailyBudget>3000 AND (grade='C' OR roas<100) AND nextBudget<dailyBudget` where `nextBudget=max(3000, roundBudget(dailyBudget*0.5))` | `change_daily_budget` | `campaign` | grade=C: `high` / else `medium` |

**변경 사항 (v1 → v2)**:
- ~~listing-level aggregate 규칙~~ 폐기. snapshot-level 단건 규칙으로 port.
- ~~`AdConfigService.getConfig(companyId)` 기반 threshold 주입~~ 부정확. 현재 AdConfig 는 SystemSetting 저장소이며 광고 rule threshold 를 담고 있지 않음. 현행 하드코딩 유지. 추후 BusinessRule 이관은 별도 plan.
- ~~`targetType:'listing'`~~ 잘못된 값. 실제는 `'campaign'` 또는 `'keyword'`.

**재고 판정 경로 변경** (v1 의 `option.availableStock` 아이디어는 맞았으나 적용 범위 오해):
- `snapshot.optionId` 존재 → `prisma.productOption.findFirst({id: optionId, companyId}).availableStock`
- `snapshot.optionId` null → Rule 1 skip (listing-level snapshot 은 option stock 판정 불가)
- BundleStockService 주입 없이 `ProductOption.availableStock` 필드 직접 read (이미 materialize 되어 있음, ADR-0014)

### 4.6 ADR-0006 compliance (`@CurrentCompany()` 전파)

**14 handler 전부** `@CurrentCompany() companyId: string` 주입. Service 메서드 시그니처에 `companyId: string` 을 **마지막 파라미터** 로 추가 (Plan B2a `InventoryController` 관례 준수).
...
`handleActionCommand` 는 dispatch 내부 7 메서드 전부 companyId 전달:
```ts
@Post('actions')
...
모든 `getDefaultCompanyId()` private helper 삭제 (총 6개 service). `prisma.company.findFirst({isActive:true})` 호출 전면 제거. `@Global() CompanyScopeGuard` (`app.module.ts:105`) 가 이미 `req.authUser.companyId` 주입 중이라 middleware 배선 추가 작업은 없음 — 데코레이터만 추가하면 동작.
...
- **`FindAllAdsResponseSchema` (신규)** — `GET /api/ads` (catch-all) 의 응답: `{ items: AdsListItem[], total: number, page: number, limit: number }`. 기존 paginated shape 보존 + listingId-primary 원소.
...
- `ad-strategy.spec.ts` — ABC grade 분류 (spend 비율 기반), 5 score 계산 (sales/review/ad/fulfillment/info), budget allocator 로직은 **public `getWeeklyPlan` 경유** 로 테스트 (private `calcBudgetAllocation` 직접 호출 금지 — apps/server/CLAUDE.md "behavior only" 규칙). `determineTopIssue` 는 listing-level input 으로 동작 (public `getRules` 경유)
- `ad-benchmark.spec.ts` — diagnosis threshold 판정 (ctr/roas/cvr 업계 평균 대비)
- `ad-sync.spec.ts` — `buildListingMap` 2-key 빌드 (vendorItemMap + externalIdMap), `matchListingFromRow` 3-tier 우선순위, upsert idempotency (같은 vendorItemId 중복 sync), **cross-tenant 분리** (company A 의 vendorItemId 가 company B sync 에 섞이지 않는지)
- `ad-action.spec.ts` — 5 규칙 generate 각 조건/비조건 + trigger 값 경계 (spend=5000 / roas=100 / roas=480 / dailyBudget=3000), `snapshot.optionId` null 일 때 Rule 1 skip, `approveActions` / `rejectActions` 일괄, IDOR guard (`findFirst({id, companyId})`), lifecycle (`markRunning` → `markDone`)

### 6.3 Controller E2E (1 file)

- `advertising.controller.spec.ts` — 14+ endpoint 각 200 / 400 (invalid body) / 404 (not found). `@CurrentCompany()` 주입 확인 (middleware 가 `req.authUser.companyId` 주입 → service 호출 시 companyId **마지막 인자** 전달 여부 검증). `handleActionCommand` 는 7 sub-action 모두 companyId 전달 검증. `registerCampaign` invalid `listingId` (다른 company 의 listing) → 404 검증
...
- `ad-action-flow.pg.integration.spec.ts`:
  - seed: listings + AdSnapshot rows (pageType=campaign / keyword 혼합, 5 규칙 trigger 조건)
  - 시나리오: generateActions → 5 rule 별 각 1+ action 생성 → approve 일괄 → markRunning → markDone → queries filter
  - cross-tenant guard: company A 의 ad action id 로 company B context 에서 approve/markRunning/markFailed 시도 → 404
  - null optionId 케이스: listing-level snapshot (optionId=null) 은 Rule 1 skip 검증
  - **out of scope note**: ExecutionTask 생성 (`ad-execution.service`) 은 변경 없음 — AdAction → ExecutionTask 파이프라인 integration 은 B2b 에서 커버하지 않음
...
| ad-flow.spec.ts 폐기 시 cross-module (advertising + agent-registry) 검증 공백 | agent-registry/domains/ad-strategy 는 Ad model 직접 touch 하지 않고 agent orchestration 만 담당. **기존 `apps/server/src/agent-registry/domains/ad-strategy/__tests__/ad-strategy.service.spec.ts` 가 `onResultReady` 경로 커버 확인 완료**. 신 advertising unit + integration 은 advertising 내부 흐름 커버 |
| ad-strategy 분할 deferring 으로 1139 LOC 단일 파일 유지 → 리뷰 부담 | 1 task = 1 commit = 1 DM cycle. ad-strategy 는 여러 task 로 분할 dispatch (예: T4a=calcActions / T4b=calcBudgetAllocation / T4c=getExposureAnalysis) — Plan 단계에서 결정. **정량 트리거**: post-port LOC > 1400 시 Plan B2b.refactor 강제 |
| 대량 integration test (4 files) DB prepare 비용 | `makeTestPrisma` + `resetDb` + `seedBaseFixture` 를 B2a 와 동일하게 재사용 |
| AdAction → ExecutionTask 파이프라인 integration 미커버 (ad-execution 변경 없음) | 기존 ad-execution 단위 테스트 (현재 어떤지 별도 확인 필요 — 없으면 가벼운 unit spec 추가 권장) 로 커버. 전체 파이프라인 end-to-end 검증은 Plan B2c 에서 다룰 것 |
| apps/web 일시 빌드 실패 (Plan D 대기) | `apps/server` + `packages/shared` 만 CI gate 에 포함. `apps/web build` 는 Plan D 완료까지 advisory. Shared rewrite 는 merge 가능 |
| buildListingMap 메모리 비용 (10k listings × 3 options = 30k+10k entries ≈ 8MB) | per-sync 수명 (GC 가능). 캐시 없음 (stale risk 회피). 현실적 크기에서 무시 가능 |
...
5. ADR-0006 violation grep: `grep -rn 'getDefaultCompanyId\|company\.findFirst.*isActive' apps/server/src/advertising` → **0 hit** (6 services 전부 clean)
6. Stale schema grep (advertising 내부): `grep -rnE 'prisma\.product\b|Ad\.productId|by:\s*\[.productId.\]|productId:\s*string' apps/server/src/advertising` → **0 hit**
7. uploads.processAdCsv 본문 stub 확인: `grep -n 'NotImplementedException' apps/server/src/uploads/uploads.service.ts` → 1+ hit
8. `src/advertising/CLAUDE.md` 에서 "Plan B2 대기" banner 제거
9. `apps/server/CLAUDE.md` Domain Guides 표에서 advertising 행 banner 제거
10. PR body 에 "Plan B2 순서" 업데이트 (B2c → B3 → D)
11. ad-strategy.service.ts post-port LOC 측정 (`wc -l`). > 1400 시 Plan B2b.refactor 를 follow-up 테이블에 Mandatory 로 표기

**`npm run dev:server` 부팅은 완료 기준에 포함하지 않는다** (dashboard/finance B3 대기).
**`apps/web` 빌드는 완료 기준에 포함하지 않는다** (Plan D 대기).
...
- **Plan B2b.refactor** (후속): ad-strategy.service.ts 의 sub-service 추출 (`ad-grade-rules` + `ad-budget-allocator` + `ad-recommend` + `ad-exposure`). `apps/server/CLAUDE.md` flag 된 refactor. **조건부**: post-port LOC > 1400 이면 **강제**, 그 외엔 Plan B3 후속 재평가.
- **Plan B2b.repo** (후속): `AdRepository` 추출 여부 재평가 — service touch 수 / 공통 include 중복 실측. 불필요하면 버림.
- **Plan B2c**: `master-product-resolver.ts` 전체 삭제 (B2a 에서 이연된 최종 정리). resolvePricing 은 B2c 의 out-of-scope 도메인들이 사용 중이라 그 시점에 대체.
- **Plan B2c**: Ad rule threshold 를 하드코딩 (5000/100/200/480/0.85/1.2/0.5/3000) 에서 `BusinessRule` 테이블 이관 검토. A/B 테스트 또는 per-company tuning 필요 시점에 집행.
- **Plan B2c**: `AdSnapshot.listingId null` cleanup 정책 — 미매칭 스냅샷의 TTL (예: 30d). ADR-0014 의 `resultRetentionDays` 유사 패턴.
- **Plan D** (후속): `apps/web/src/app/ad-ops/*` 재배선. shared/ads.ts 신 schema consume. RegisterCampaignDto 의 `products` → `listings` 이름 변경 반영.
...
## 13. Open questions (v2 에서 대부분 resolve)

- ~~**AdAction.targetType** 관례 문서화~~ → **Resolved**: `'campaign' | 'keyword'` 값으로 확정. `services/types.ts` 에 `AD_ACTION_TARGET_TYPES` const 선언 + CLAUDE.md 명시.
- ~~**ad-strategy 분할 시점**~~ → **Resolved**: 정량 트리거 (post-port LOC > 1400) 도입.
- **`AdSnapshot.listingId` nullable cleanup 정책**: 미매칭 snapshot TTL 정책 필요. B2b 범위 밖, Plan B2c 로 이월.
- **Ad rule threshold 이관**: 5000/100/200/480/0.85 등 하드코딩 값의 `BusinessRule` 이관 여부. B2c 이후.
- **ad-execution 단위 테스트**: 현재 존재 여부 미확인. Plan 작성 시 grep 으로 확인 → 없으면 lightweight unit spec 1개 추가 (lease/heartbeat/report lifecycle) 를 Plan B2b 범위에 포함할지 결정.
