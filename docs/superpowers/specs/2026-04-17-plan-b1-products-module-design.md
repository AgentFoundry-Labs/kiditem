...
모든 endpoint 는 **전역 `CompanyScopeGuard`** (`APP_GUARD` via `app.module.ts`) + `@CurrentCompany()` decorator 사용 (ADR-0006, `auth/CLAUDE.md` "Hard bans" — controller 레벨 `@UseGuards` 금지). 전역 `ValidationPipe({whitelist:true, transform:true})` 는 `main.ts` 에 등록되어 있으므로 **controller 에서 `@UsePipes` 사용 금지**.
...
import { MasterSchema, MasterWithOptionsSchema, type Master, type MasterWithOptions } from '@kiditem/shared';
import { toSerializable } from '../util/serialize';  // apps/server/src/products/util/serialize.ts
...
export class MastersController {
  ...
  @Get(':id')
  async findById(...): Promise<MasterWithOptions> { ... }     // relation-loaded
  ...
  @Get('by-code/:code')
  async findByCode(...): Promise<Master> { ... }
...
**No `@UseGuards`, no `@UsePipes`** — 전역 guard + pipe 사용. `orders/orders.controller.ts` 등 기존 패턴과 일치.

**Services 는 raw Prisma row 반환** → **Controllers 가 Zod 로 serialize**. `toSerializable()` 은 `apps/server/src/products/util/serialize.ts` 에 위치 (`packages/shared` 는 `@prisma/client` 에 의존하지 않음 — duck-typed Decimal check 사용, **Prisma 모듈 import 금지**).
...
**Helper**: `apps/server/src/test-helpers/real-prisma.ts` 의 `makeTestPrisma`, `resetDb`, `seedBaseFixture` 재사용. Service 는 직접 instantiate (전역 guard / pipe / controller 우회).
...
`products.module.di.spec.ts` (vitest, not pg.integration — 순수 NestJS DI). Controller 가 `@UseGuards` 쓰지 않으므로 `overrideGuard` 불필요:
...
| 전역 `CompanyScopeGuard` real 검증 path (HTTP req 시나리오) | Service 직접 호출 — companyId 파라미터로 주입 | Plan B3 HTTP integration |
