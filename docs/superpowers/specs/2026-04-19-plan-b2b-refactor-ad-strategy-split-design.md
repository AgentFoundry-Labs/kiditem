# Plan B2b.refactor — Ad Strategy Service Split (Design Spec v1)

> **Status**: Draft v1 — pending architect review (light)
> **Session**: 2026-04-19
...
| `apps/server/src/advertising/services/ad-grade-rules.service.ts` | **Create**. `@Injectable()` class. **Prisma 의존 없음**. Public method: `calcActions(input: GradeRulesInput): AdStrategyAction[]`, `calcAdIssues(input: AdIssuesInput): AdIssues`. Private helper: `ruleToActionType(rule)`. Input 타입 (`GradeRulesInput`, `AdIssuesInput`) 은 `services/types.ts` 에 신규. 기존 ad-strategy.service.ts 의 line 653-941 (calcActions, 288 LOC) + line 941-955 (ruleToActionType) + line 955-1028 (calcAdIssues) 코드 그대로 이전. 동작 변경 없음. |
| `apps/server/src/advertising/services/ad-budget-allocator.service.ts` | **Create**. `@Injectable()` class. **Prisma 의존 없음**. Public method: `calcBudgetAllocation(input)`, `calcTierAnalysis(input)`, `calcSnapshotKeyMetrics(input)`, `calcTop20(input)`. Input 타입은 types.ts 에 신규. 기존 line 557-609 (calcSnapshotKeyMetrics) + 609-653 (calcBudgetAllocation) + 1028-1067 (calcTierAnalysis) + 1067-1150 (calcTop20) 이전. |
...
NestJS DI 우회 — orchestrator 가 자신의 PrismaService 를 함수에 전달. 이 패턴은 channels/adapters 에도 사용 중이라 컨벤션 위반 아님.
...
- **calcSnapshotKeyMetrics 위치**: budget-allocator 에 두지만 grade-rules 도 gradeMap 필요. orchestrator 가 한 번 호출 후 양쪽에 전달 (선호) vs 두 곳 모두 계산 (중복). → orchestrator 가 budget-allocator 의 calcSnapshotKeyMetrics 호출 결과를 grade-rules 에도 전달 (단일 출처).
- **ad-strategy.spec.ts 의 운명**: 5-10 thin delegation 유지 vs 전체 drop. → Plan 단계에서 결정. 추천: 5 thin delegation 유지 (배선 회귀 detection).
