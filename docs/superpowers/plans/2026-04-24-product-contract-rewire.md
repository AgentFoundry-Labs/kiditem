...
- Legacy alias is **read-only** in this slice: `GET` list / `GET :id` detail / `GET pipeline-stats` / `GET :id/original-image-base64` + both `GET` and `POST` for `calculate-grades` (the `POST` is the `action-task.service.ts:411` scheduled caller; it returns current catalog counts with no grade write). `PATCH` / `PUT` are **not registered**. `workflows/actions/catalog.ts` previously POSTed 4 product write actions through `PUT /api/products/{productId}` — those 4 action templates are **removed in this slice** so the LLM no longer proposes them and no execution path 404s. Canonical write replacements ship with the agent/workflow redesign (see §Deferred Work).
