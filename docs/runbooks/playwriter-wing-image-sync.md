...
`/product-pipeline/thumbnail-generation` 페이지의 **이미지 동기화** 버튼은 쿠팡 Wing 인벤토리
...
쿠팡 Wing 로그인 탭이 있으면 `/product-pipeline/thumbnail-generation` 이 extension 에 수집을 요청하고,
...
남는다. 사람이 로그인한 뒤 `/product-pipeline/thumbnail-generation` 에서 **이미지 동기화** 를 다시 누른다.
...
staging `/product-pipeline/thumbnail-generation` 를 새로고침해야 page -> extension handshake 가 갱신된다.
...
- `apps/web/src/app/(product-pipeline)/product-pipeline/_shared/lib/wing-registration.ts` — prepare → extension → complete browser flow.
...
cd ../web
npx vitest run "src/app/(product-pipeline)/product-pipeline/_shared/lib/wing-registration.spec.ts"
...
1. Local: `http://localhost:3000/product-pipeline/thumbnail-generation` 또는 staging:
   `<STAGING_URL>/product-pipeline/thumbnail-generation` 접속.
2. 로그인 상태 확인.
3. **이미지 동기화** 클릭.
4. 결과 확인.
...
- Frontend: /product-pipeline/thumbnail-generation button verified
