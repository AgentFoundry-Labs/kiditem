...
전달한다. Extension 이 감지되지 않는 **local 개발 환경**에서만 backend
Playwriter fallback 이 managed Chrome/CDP session 을 만든다. Staging 처럼
배포된 origin 에서는 extension 경로만 허용한다.
...
이때 extension 이 감지되지 않으면 frontend 는 server fallback 을 호출하지 않고
extension 리로드 안내 오류를 표시한다.
