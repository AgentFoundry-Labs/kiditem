...
```bash
# 1. .env / apps/web/.env.local 에 받은 SUPABASE_* 키 4개 setting (PR description 또는 별도 secret 채널)
...
3. **키 4개 복사** — Settings → API
   - Project URL
   - anon public key
   - service_role secret key (절대 프론트/git 노출 금지)
   - JWT Settings → JWT Secret

4. **`.env` / `.env.local` 작성** — 사용자가 직접 파일에 붙여넣기
...
- `apps/web/src/middleware.ts` — `/login` 리다이렉트 가드
...
- `SUPABASE_SERVICE_ROLE_KEY` 는 sync 스크립트 전용. 절대 프론트/git/PR description 에 노출 금지.
