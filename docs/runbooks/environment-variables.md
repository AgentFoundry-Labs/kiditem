...
| `AI_PROVIDER_TIMEOUT_MS` | Direct AI provider calls need a non-default execution budget | AI direct-job worker | Optional; defaults to `120000`. Must be a positive integer. Timeout aborts the execution signal and is retryable. Gemini SDK calls also carry a native 120-second HTTP timeout. |
