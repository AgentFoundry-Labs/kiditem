...
verify_prompt_scope() {
  ...
  local kiditem_hits
  kiditem_hits="$(cd "$KIDITEM_ROOT" && "$CODEX_BIN" debug prompt-input probe 2>/dev/null | perl -pe 's/\\n/\n/g' | grep -E '^- (agents-md-audit|caveman|grill-me|improve-codebase-architecture|kiditem-market-sourcing-radar|magic-scraper|staging-deploy-operator|supabase|supabase-postgres-best-practices|vercel-react-best-practices|update-project-skills)' | head -20 || true)"
