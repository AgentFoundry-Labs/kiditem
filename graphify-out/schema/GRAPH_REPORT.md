...
- [[_COMMUNITY_Supply schema|Supply schema]]
- [[_COMMUNITY_Supply schema|Supply schema]]
- [[_COMMUNITY_prisma field Order.id|prisma field: Order.id]]
- [[_COMMUNITY_AI schema|AI schema]]
...
Cohesion: 0.15
Nodes (16): Database ERD, Advertising, AdAction.externalId, AdAction.listing, AdAction.organization, AdAction.targetType, ChannelListing.master, Order.listing (+8 more)
...
### Community 49 - "prisma field: Order.id"
Cohesion: 0.67
Nodes (3): Order.id, orders, Order unique(id, organizationId)
...
- **Why does `Database ERD` connect `Advertising schema` to `Core schema`, `Core schema`, `AI schema`, `System schema`, `Inventory schema`, `System schema`, `Core schema`, `AgentOS schema`, `Orders schema`, `Finance schema`, `Sourcing schema`, `AgentOS schema`, `System schema`, `Advertising schema`, `Inventory schema`, `Orders schema`, `Supply schema`, `AgentOS schema`, `AgentOS schema`, `AI schema`, `Orders schema`, `AI schema`, `Channels schema`, `Channels schema`, `Channels schema`, `Advertising schema`, `Finance schema`, `Core schema`, `Orders schema`, `AgentOS schema`, `Core schema`, `Inventory schema`, `Orders schema`, `Supply schema`, `AgentOS schema`, `AgentOS schema`, `AI schema`, `Orders schema`, `AgentOS schema`, `Channels schema`, `Core schema`, `Core schema`, `Orders schema`, `Supply schema`, `AgentOS schema`, `Channels schema`, `Supply schema`, `Supply schema`, `prisma field: Order.id`, `AI schema`?**
  _High betweenness centrality (0.550) - this node is a cross-community bridge._
- **Why does `Organization` connect `Core schema` to `Core schema`, `AI schema`, `System schema`, `Inventory schema`, `System schema`, `Core schema`, `AgentOS schema`, `Orders schema`, `Finance schema`, `Sourcing schema`, `AgentOS schema`, `System schema`, `Advertising schema`, `Inventory schema`, `Orders schema`, `Supply schema`, `AgentOS schema`, `AgentOS schema`, `AI schema`, `Orders schema`, `AI schema`, `Channels schema`, `Channels schema`, `Channels schema`, `Advertising schema`, `Finance schema`, `Advertising schema`, `Orders schema`, `AgentOS schema`, `Core schema`, `Inventory schema`, `Orders schema`, `Supply schema`, `AgentOS schema`, `AgentOS schema`, `AI schema`, `Orders schema`, `Channels schema`, `Core schema`, `Core schema`, `Orders schema`, `Supply schema`, `AgentOS schema`, `Channels schema`, `AI schema`?**
