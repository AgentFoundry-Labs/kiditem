...
- `CLAUDE.md` = 현재 시점의 규칙 ("지금 이렇게 한다")
- `.claude/docs/{erd,architecture,workflow,commands}.md` = 현재 상태 스냅샷
- `.claude/docs/lessons.md` = 교훈 ("다시는 이러지 말자")
...
- `## Related` — 관련 ADR 번호, `lessons.md` 엔트리, 메모리 노트, PR 링크
...
동일 사건은 **ADR 이 정본**. `lessons.md` 는 결정으로 이어지지 않는 프로세스 교훈(예: DI 검증 관성, follow-up debt 지양) 만 담는다. 같은 사건이 양쪽에 있으면 `lessons.md` 엔트리를 `→ [ADR-NNNN](decisions/NNNN-...md)` 한 줄 포인터로 축약.
...
### 영향 없음 (현재 시점)
`apps/server/src/workflows`, `apps/server/src/advertising`, `apps/web`, `packages/shared`, `packages/templates` — 해당 도메인 규칙이 생성·전복되는 시점에 ADR 발행.
