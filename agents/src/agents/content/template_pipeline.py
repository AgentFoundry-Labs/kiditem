...
_SIZE_CHART_PROMPT = (
    ...
    "1. 추출: 치수선, 화살표, 숫자(cm, mm)가 표시된 제품/포장 사진만 포함하세요.\n"
    "   제거: 치수 표시가 없는 순수 제품 쇼케이스 사진, 장식 텍스트, 배경.\n"
    "   예: 같은 이미지 안에 치수 없는 제품 사진과 치수 있는 사진이 함께 있으면,\n"
    "       치수가 있는 사진만 남기세요.\n\n"
    "2. 번역: 치수 관련 중국어 라벨만 한국어로 바꾸세요.\n"
    "   예: 高度 → 높이, 宽 → 너비, 长 → 길이, 直径 → 지름, 产品包装 → 제품 포장.\n"
    "   번역 금지: 상품 자체에 인쇄/각인된 텍스트, 브랜드명, 로고.\n\n"
    "3. 레이아웃: 추출한 사진들을 세로로 배치하세요.\n"
    "   - 추출한 각 사진은 잘리지 않게 전체를 포함하세요.\n"
    "   - 사진이 2개 이상일 경우, 사진 사이에 최소 150px 이상의 흰색 빈 공간을 반드시 두세요.\n\n"
    "중요:\n"
