...
export const migrateRepresentativeKeywordOverrides: DataMigration = {
  ...
  async run(tx) {
    ...
    const ownProducts =
      allIds.length > 0
        ? await tx.coupangProductListing.findMany({
            where: { vendorItemId: { in: allIds } },
            select: { organizationId: true, vendorItemId: true },
          })
        : [];
    const ownKeys = new Set(
      ownProducts.flatMap((row) =>
        row.vendorItemId
          ? [`${row.organizationId}\u0000${row.vendorItemId}`]
          : [],
      ),
    );
